...
```{r}
mean_amplitude <- data[, abs_speed_above_10] %>% future_apply(1, mean)
...
```{r}
n = car_trigger_signal_smoothing_factor = 31

car_trigger_signal <- runner(mean_amplitude, f=weighted.mean, k=n, lag=-(n-1)/2, w=dnorm((-(n-1)/2):((n-1)/2),sd=(n-1)/8), na_pad = T)
...
while(i <= length(car_trigger_direction_signal)){
  if(is.na(car_trigger_direction_signal[i]) | car_trigger_direction_signal[i] < diff_threshold){
    ...
    if(trigger_max > -trigger_min &  (trigger_end - trigger_start)>10){
    ...
    if(trigger_max < -trigger_min &  (trigger_end - trigger_start_shorter)>10){
      # max_speeds["max_i",trigger_start:trigger_end] <- NA
      # max_speeds["max_i_rev",trigger_start_shorter:trigger_end] <- NA
